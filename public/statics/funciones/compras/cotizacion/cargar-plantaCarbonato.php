...
<div class="container-fluid" style="width:99%">
    <div class="row">
        <h3 class="nombre-tabla ocultar"> Planta Carbonato</h3>
        <table class="display nowrap table-bordered table-hover" style="width:100%;" id="tablaPlantaCarbonato">
            <thead>
            ...
            <tbody>
            <?php
            $sql = "SELECT * FROM ordenPedido WHERE bodega = 1 ORDER BY codigo DESC;";
            $result = mysqli_query($con, $sql);
            $url = '';
            if(isset($_GET['estado'])){
                $url = '../compras/orden-compra.php?area=1&estado=cotizado&codigo=';
            }else{
                $url = '../compras/cotizacion.php?area=1&codigo=';
            }
...
<style type="text/css">
    ...
    @media (max-width: 375px) {
        ...
        #tablaPlantaCarbonato_filter label{
            margin-top: 5px;
            float: left;
            width: 90%;
        }
        #tablaPlantaCarbonato_filter label .input-sm{
            width: 90%;
        }
        #tablaPlantaCarbonato_filter{
            margin-top: 10px;
        }
    }
</style>
<script>
    $(document).ready(function() {
        $('#tablaPlantaCarbonato').DataTable( {
            "order": [[ 0, 'desc' ]],
            "scrollY":        "300px",
            "scrollCollapse": true,
            "paging":         false,
        ...
        var table = $('#tablaPlantaCarbonato').DataTable();
       
        $('#tablaPlantaCarbonato tbody').on( 'click', 'tr', function () {
