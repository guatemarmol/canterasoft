...
<div class="container-fluid" style="width:99%">
    <div class="row">
        <h3 class="nombre-tabla ocultar">Predio Trasiego</h3>
        <table class="display nowrap table-bordered table-hover" style="width:100%;" id="tablaPredioTrasiego">
            <thead>
            ...
            <tbody>
            <?php
            $sql = "SELECT * FROM ordenPedido WHERE bodega = 4 ORDER BY codigo DESC;";
            $result = mysqli_query($con, $sql);
            $url = '';
            if(isset($_GET['estado'])){
                $url = '../compras/orden-compra.php?area=4&estado=cotizado&codigo=';
            }else{
                $url = '../compras/cotizacion.php?area=4&codigo=';
            }
...
<style type="text/css">
    ...
    @media (max-width: 375px) {
        ...
        #tablaPredioTrasiego_filter label{
            margin-top: 5px;
            float: left;
            width: 90%;
        }
        #tablaPredioTrasiego_filter label .input-sm{
            width: 90%;
        }
        #tablaPredioTrasiego_filter{
            margin-top: 10px;
        }
    }
</style>
<script>
    $(document).ready(function() {
        ...
        var table = $('#tablaPredioTrasiego').DataTable();
       
        $('#tablaPredioTrasiego tbody').on( 'click', 'tr', function () {
