...
<div class="container-fluid" style="width:99%">
    <div class="row">
        <h3 class="nombre-tabla ocultar">Taller</h3>
        <table class="display nowrap table-bordered table-hover" style="width:100%;" id="tablaTaller">
            <thead>
            ...
            <tbody>
            <?php
            $sql = "SELECT * FROM ordenPedido WHERE bodega = 9 ORDER BY codigo DESC;";
            $result = mysqli_query($con, $sql);
            $url = '';
            if(isset($_GET['estado'])){
                $url = '../compras/orden-compra.php?area=9&estado=cotizado&codigo=';
            }else{
                $url = '../compras/cotizacion.php?area=9&codigo=';
            }
...
<style type="text/css">
    ...
    @media (max-width: 375px) {
        ...
        #tablaTaller_filter label{
            margin-top: 5px;
            float: left;
            width: 90%;
        }
        #tablaTaller_filter label .input-sm{
            width: 90%;
        }
        #tablaTaller_filter{
            margin-top: 10px;
...
<script>
    $(document).ready(function() {
        ...
        $('#tablaTaller tbody').on( 'click', 'tr', function () {
