...
<div class="container-fluid" style="width:99%">
    <div class="row">
        <h3 class="nombre-tabla ocultar"> Producción de Piedra</h3>
        <table class="display nowrap table-bordered table-hover" style="width:100%;" id="tablaProduccionPiedra">
            <thead>
            ...
            <tbody>
            <?php
            $sql = "SELECT * FROM ordenPedido WHERE bodega = 2 ORDER BY codigo DESC;";
            $result = mysqli_query($con, $sql);
            $url = '';
            if(isset($_GET['estado'])){
                $url = '../compras/orden-compra.php?area=2&estado=cotizado&codigo=';
            }else{
                $url = '../compras/cotizacion.php?area=2&codigo=';
            }
...
<style type="text/css">
    ...
    @media (max-width: 375px) {
        ...
        #tablaProduccionPiedra_filter label{
            margin-top: 5px;
            float: left;
            width: 90%;
        }
        #tablaProduccionPiedra_filter label .input-sm{
            width: 90%;
        }
        #tablaProduccionPiedra_filter{
            margin-top: 10px;
        }
    }
</style>
<script>
    $(document).ready(function() {
        $('#tablaProduccionPiedra').DataTable( {
            "order": [[ 0, 'desc' ]],
            "scrollY":        "300px",
            "scrollCollapse": true,
            "paging":         false,
        ...
        var table = $('#tablaProduccionPiedra').DataTable();
       
        $('#tablaProduccionPiedra tbody').on( 'click', 'tr', function () {
