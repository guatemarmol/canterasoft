<?php
    ...
    if($fechaI != '1970-01-01'){
        $query = "SELECT O.descripcion, O.unidadMedida, O.cantidad, C.precio, C.descuento, C.precioFinal, C.precioOrden
    ...
    $headerInfo = ["PROVEEDOR", "", "PERÍODO", "", "FECHA"];
    

    if($fechaI != '1970-01-01'){
        $infoData = [$proveedor,"", "$fechaI - $fechaF", "", "$fecha",];
    }else{
        $infoData = [$proveedor,"", "    -    ", "", "$fecha",];
    }

    $headerTable = ["No.","DESCRIPCIÓN","PRECIO\nUNITARIO","DESCUENTO","PRECIO\nFINAL","SUB\nTOTAL\nQTZ"];
    ...
    $writer->save('ReporteProveedor-'.$proveedor.'.xlsx');
    echo 'ReporteProveedor-'.$proveedor.'.xlsx';
?>