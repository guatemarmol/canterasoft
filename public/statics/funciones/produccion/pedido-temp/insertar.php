...
<?php
$codigo   = $_POST['codigo'];
$consulta = "SELECT * FROM articuloTransaccion WHERE codigo='$codigo'";
...
if ($fila > 0) {
    ...
    $fecha       = $_POST['fecha'];
    $hora        = $_POST['hora'];
    $actualizar  = "UPDATE articuloTransaccion SET descripcion='$descripcion', fecha='$fecha', hora='$hora'  WHERE codigo='$codigo'";
    ...
    $consulta = "SELECT MAX(codigo) codigo FROM articuloTransaccion";
    $ejecutar = mysqli_query($con, $consulta);
    $tabla    = mysqli_fetch_array($ejecutar);
    $codigo   = $tabla['codigo'];
    $codigo++;
    if ($codigo == 1) {
        $codigo = "ARTT-0000000000";
    } else if ($codigo != 1) {
        $codigo = $codigo;
    }
    $descripcion = $_POST['descripcion'];
    $fecha       = $_POST['fecha'];
    $hora        = $_POST['hora'];
    $agregar     = "INSERT INTO articuloTransaccion (codigo, descripcion, fecha, hora)
